...
Let $S = \{l,q\}$ be a set of two primes which are both different from 3.
The $S$-integral points of the thrice-punctured line carry an $S_3$-action
generated by $z \mapsto 1-z$ and $z \mapsto 1/z$. In [BBKLMQSX] we show that 
each $S$-integral point has an $S_3$-translate which satisfies the refined 
Selmer equation
...
The function "check_extrapoint_criterion" determines all pairs of primes $l$, $q$
...
- [BBKLMQSX] Alex Best, L. Alexander Betts, Theresa Kumpitsch, Martin Lüdtke,
  ...
  equations for the thrice-punctured line in depth two" (2021)
...
def check_extrapoint_criterion(bound, prec):
    r"""
    Check criterion from [BBKLMQSX, Prop. 3.12] characterizing for which $S = \{l,q\}$
    the refined CK-set in depth 2 with $p=3$ contains a point besides $\{-1,2,1/2\}$.

    INPUT:

    - ``bound`` -- upper bound for primes l, q
    ...
    three lists of pairs (l,q) of primes l,q < bound with $l,q \neq 3$ such that
    the refined Chabauty--Kim set in depth 2 for S = {l,q} and p=3...
    ...
    Check the criterion for l,q < 20:
    ...
    The algorithm for the bound 500 runs in under 10 minutes:
        ...
        sage: extrapoint, noextrapoint, undecided = check_extrapoint_criterion(bound=500,prec=10)
        sage: len(extrapoint)
        2841
    ...
    for iq in range(len(prime_list)):
        ...
        for il in range(iq):
            ...
            if l == 3:
                ...
                a = three_adic_dcw_coefficient(l, q, dec, prec, logs, logs_one_minus, dilogs)
                b = prime_logs[l] * prime_logs[q] - a
                val_a = a.valuation(3)
                val_b = b.valuation(3)
                val_log_l = (l-1).valuation(3) if l % 3 == 1 else (l+1).valuation(3)
                val_log_q = (q-1).valuation(3) if q % 3 == 1 else (q+1).valuation(3)
                if (val_a >= prec or val_b >= prec):
                    undecided.append((l,q))
                elif min(val_a,val_b) == val_log_l + val_log_q:
                     extrapoint.append((l,q))
                else:
                     noextrapoint.append((l,q))
